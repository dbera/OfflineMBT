...
TEMP_FILE = tempfile.TemporaryDirectory(prefix=prefix_str, ignore_cleanup_errors=True)
...
def build_and_load_model(model_path: str) -> Tuple[Any, subprocess.CompletedProcess]:
    ...
    if result.returncode != 0:
        raise utils.BPMN4SException(cliargs={"bpmn-file": model_name}, result=result)
...
def generate_tests(
    model_path: str, num_tests: int = 1, depth_limit: int = 500
...
def test_generator() -> Tuple[Any, int]:
    ...
    try:
        ...
        return (
            send_from_directory(zip_dir, zip_path, mimetype="application/zip", as_attachment=True),
            status_code,
        )
    except utils.BPMN4SException as e:
        status_code = 400
...
def handle_transitions_enabled(uuid: str) -> Tuple[Any, int]:
    ...
    pn = utils.get_cpn(uuid)
    status_code = 200
    response = {"id_mode_dict": {}, "id_transition_dict": {}}
    try:
