...
class ProductParsingTest {
    ...
    @Inject
      extension Provider<ResourceSet> resourceSetProvider
    
    @BeforeAll
    def void setup() {
        var set = resourceSetProvider.get()
        set.createResource(URI.createURI("printer_v6.types")).load(new StringInputStream('''
            record UNIT {
                int unit
            }
            ...
            enum PrintResolution { LOW MED HIGH }
            ...
            record CorrectionsReport {
                map<int,CorrectionItem[]>   correctionsMap
                int id
            }
            
            enum Outcome { OK NOK }
            
            enum ColorType { MONOCHROME COLOR }
            
            enum OperationType { PREP PRINT }
        '''), emptyMap)
    }
    ...
    def void loadModel() {
        val result = parseHelper.parse('''
          import "printer_v6.types"
          specification printer_v6Final
          {
              ...
              system PrintFactoryOptimization
              {
                  ...
                  updates:    corrections := CorrectionsReport {
                       correctionsMap = 
                                        ...
                                        0 -> <CorrectionItem[]>[ CorrectionItem { data = "" } ] 
                      },
                       id = optimizeJob.id + 1
                      }
              ...
              system PrintFactoryFactoryAutomation
              {
                  ...
                  case            default step-type "null" action-type ASSERT
