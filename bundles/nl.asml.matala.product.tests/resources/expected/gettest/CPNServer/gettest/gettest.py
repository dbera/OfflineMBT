...
if __package__ is None or __package__ == '':
    ...
    from gettest_reporting import get_reporting, initialize_reporting, Location, Severity
...
class gettestModel:
    ...
    def __init__(self):
        ...
        self.n = PetriNet('gettest')
        self.n.globals["Data"] = Data
        self.n.globals.declare("import json")
        self.n.add_place(Place('test'))
        self.n.add_place(Place('single'))
        self.n.place('test').empty()
        self.n.place('test').add(json.dumps({"aMap": {1: "a", 2: "b", 3: "c"}, "aList": [1, 2, 3]}))
        self.n.add_transition(PrioritizedTransition('Root_T1_default@null@INTERNAL@', 0))
        self.n.add_input('test','Root_T1_default@null@INTERNAL@',Variable('v_test'))
        self.n.add_output('single','Root_T1_default@null@INTERNAL@', Expression('Data().execute_Root_T1_default_single(json.loads(v_test, object_pairs_hook=Data().int_keys))'))
