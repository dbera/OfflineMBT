...
//    }
	...
	static String printAssertionsMustache(List<AssertThatBlock> assertList) {
		String assertionStr = "";
		try {
			String mustacheTemplate = """
					...
					{{#.}}
					{{#value}}{
						"id":"{{getIdentifier}}",  "type":"{{#getVal}}Value{{/getVal}}{{#getXpath}}XPaths{{/getXpath}}{{#XmlFiles}}XMLFile{{/XmlFiles}}"
						"output":"{{output}}",
						{{#getVal.comparisonType}}
						"reference":"{{reference}}",
						{{/getVal.comparisonType}}
						{{assertRef}}
					}{{/value}}{{^last}}, {{/last}}
			...
			mustache.execute(sWriter, new DecoratedCollection<>(assertList)).flush();
			assertionStr = sWriter.toString();
		} catch (IOException e) {
			e.printStackTrace();
		}
