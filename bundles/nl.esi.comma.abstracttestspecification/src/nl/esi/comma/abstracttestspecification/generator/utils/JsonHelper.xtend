...
class JsonHelper {
        ...
        return switch (elem) {
            JsonExpression: {
                ...
                switch (expr) {
                    ...
                    ExpressionMinus: Utils.getStringSignedValue(expr)
                    ExpressionPlus: Utils.getStringSignedValue(expr)
                    default: throw new IllegalArgumentException('Unknown Expression type ' + expr)
                }
            }
            JsonObject: jsonElement(elem as JsonObject)
            JsonArray: jsonElement(elem as JsonArray)
            default:
    ...
    def static String toXMLElement(JsonValue elem, Map<String, String> rename) {
        switch (elem) {
            JsonObject:
                return toXMLElement(elem, rename)
            JsonArray:
                return toXMLElement(elem, rename)
            JsonExpression: {
                val expr = elem.expr
