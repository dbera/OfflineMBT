...
public class ProposalHelper {
	...
	public static String defaultValue(Type type) {
		if (type instanceof TypeReference) {
			...
			return "<"+type.getType().getName()+"[]>["+defaultValue(type.getType())+"]";
		} else if (type instanceof MapTypeConstructor) {
			String key = defaultValue(type.getType()).toString();
			String value = defaultValue(((MapTypeConstructor)type).getValueType().getType());
			return "<map<" + type.getType().getName() + ", " + ((MapTypeConstructor)type).getValueType().getType().getName() 
					+ ">>{"+ key +" -> "+ value + "}";
		}
	...
	static String defaultValue(TypeDecl type) {
		if (type instanceof SimpleTypeDecl) {
			...
			String value = ((RecordTypeDecl) type).getFields().stream()
				.map(f -> String.format("%s = %s", f.getName(), defaultValue(f.getType())))
