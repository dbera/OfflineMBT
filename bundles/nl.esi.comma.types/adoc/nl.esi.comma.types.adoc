...
Vector types are constructed by giving a base type and a number of dimensions. For each dimension, an optional size can be given.
...
Every type has a default value. It is usually used to automatically initialize variables that are not explicitly given a value.
Below are the default values for each type.
