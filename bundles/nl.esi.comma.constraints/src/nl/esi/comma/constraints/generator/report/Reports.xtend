...
class Reports {
	...
	static def String toJson(ConformanceReport report) {
	...
	def byte[] getHtmlTemplate() {
		var in = this.class.classLoader.getResourceAsStream("conformance.html")
		if (in !== null) {
			try {
			...
			System.err.println('Could not load the conformance.html template from the ' + Reports.toString + ' plugin')
		}
